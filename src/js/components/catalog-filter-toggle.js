...
if (vars.$catalogChoice) {
    ...
    vars.$catalogFilterItems.forEach(el => {
        el.querySelector('input').addEventListener('change', (e) => {
            ...
            if (activeCheckboxes.length > 0) {
                ...
                vars.$catalogChoice.querySelector('.catalog-choice__clear').style.display = 'inline-block';
            } else {
    ...
    vars.$catalogChoice.addEventListener('click', (e) => {
        if (e.target.classList.contains('catalog-choice__item')) {
            ...
            if (vars.$catalogChoice.children.length == 1 ) {
                document.querySelector('.catalog-choice__clear').style.display = 'none';
            }
            ...
            document.querySelector(`[data-text="${text}"]`).closest('.catalog-filter').querySelector('.catalog-filter__quantity').textContent -= 1;
        }
    ...
    vars.$filtersOpenMobile.addEventListener('click', (e)=> {

        vars.$catalogFilters.classList.toggle('catalog-filters--active');
